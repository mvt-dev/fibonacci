...
import { InvestmentInterface, TransactionInterface } from '@fibonacci/interfaces';
...
export default class InvestmentModel extends DbModel {
  ...
  async list(endDate: Date): Promise<InvestmentInterface.Investment[]> {
    const BUY = TransactionInterface.TransactionType.Buy;
    const SELL = TransactionInterface.TransactionType.Sell;
    const query = this.db('ledger')
      ...
      .select(
        ...
        this.db.raw(`(
          ...
          AND (
            ledger_profit.type = '${TransactionInterface.TransactionType.Profit}'
            OR ledger_profit.type = '${TransactionInterface.TransactionType.JCP}'
            OR ledger_profit.type = '${TransactionInterface.TransactionType.Dividend}')
        ) AS profit`),
        this.db.raw(`(
